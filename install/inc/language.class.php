...
class Language {
	...
	public function get_available_list( $langDir = NULL ){
		$langFileList = "";
		$langDir = ( ! is_null( $langDir ) ) ? $langDir : $this->langDir;
		if($handle = opendir($langDir)){
			while (false !== ($file = readdir($handle))){
				if ($file != "index.php" && $file != "." && $file != ".." && strtolower(substr($file, strrpos($file, '.') + 1)) == substr($this->langFileExtension, strrpos($this->langFileExtension, '.') + 1)){
  ...
  public function details( $keyword, $language = NULL ) {
	$language = ( ! is_null( $language ) ) ? $this->langDir . $language . $this->langFileExtension : $this->langFile;
    include( $language );
